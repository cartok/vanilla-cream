...
export default class WizardTabPresenter {
    ...
    // @external
    // @prototype: used Function.equals prototype extension before. Had trouble when using librarys that override Prototype aswell.
    on(event: String | Array<String>, cb: Function){
        switch(event){
            case "before-activate":
                if(this.activationHooks.before.find(f => (f === cb || f.toString() === cb.toString()) === undefined)){
                    this.activationHooks.before.push(cb)
                }
                break
            case "after-activate":
                if(this.activationHooks.after.find((f === cb || f.toString() === cb.toString()) === undefined)){
        ...
        console.log(this.activationHooks.after)
    }
